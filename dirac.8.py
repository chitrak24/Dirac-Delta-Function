...
#Representation using Logarithm of Coth function
#\epsilon ---> 0
#func =  np.log(np.abs(np.cosh(x/\epsilon)/np.sinh(x/\epsilon)))
...
#Representation of Dirac Delta function using Logarithm of Coth function
def ddf(x,eps):
    return np.log(np.abs(np.cosh(x/eps)/np.sinh(x/eps)))
...
#evaluating intrgral
xp = np.linspace(a-10*eps,a+10*eps,1000)
